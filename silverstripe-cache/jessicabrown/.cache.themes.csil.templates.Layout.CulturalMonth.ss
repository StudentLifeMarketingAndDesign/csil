<?php
if ($scope->locally()->hasValue('BackgroundImage', null, true)) { 
$val .= '
<div class="main-image">
	<div class="container">
		<img class="b-lazy" data-src="';

$val .= $scope->locally()->obj('BackgroundImage', null, true)->XML_val('URL', null, true);
$val .= '" src="data:image/gif;base64," />
	</div>
</div>
';


}
$val .= '

<div class="row">

	<div class="large-12 large-centered columns">
		<ul class="breadcrumbs">
			<li><a href="';

$val .= $scope->locally()->XML_val('Baseref', null, true);
$val .= '">Home</a></li>
			<li><a href="multicultural/">Multicultural</a></li>
			<li><a href="multicultural/month">Celebration Months / Weeks</a></li>
			<li>';

$val .= $scope->locally()->XML_val('MenuTitle', null, true);
$val .= '</li>
		</ul>
		<h1>';

$val .= $scope->locally()->XML_val('Title', null, true);
$val .= '</h1>
		';

if ($scope->locally()->hasValue('EventList', null, true)) { 
$val .= '
		<h2>Events related to ';

$val .= $scope->locally()->XML_val('Title', null, true);
$val .= ':</h2>
		<ul class="features">
			';

$scope->locally()->obj('EventList', null, true); $scope->pushScope(); while (($key = $scope->next()) !== false) {
$val .= '
			<li>
				<a href="';

$val .= $scope->locally()->XML_val('AfterClassLink', null, true);
$val .= '" target="_blank"><span class="visuallyhidden">More information about ';

$val .= $scope->locally()->XML_val('Title', null, true);
$val .= '</span>
				<div class="feature-image lazy" ';

if ($scope->locally()->obj('Image', null, true)->hasValue('URL', null, true)) { 
$val .= 'data-src="';

$val .= $scope->locally()->obj('Image', null, true)->XML_val('URL', null, true);
$val .= '"';


}
$val .= ' >
					
				</div>
			</a>
			<div class="feature-content">
				<h3><a href="';

$val .= $scope->locally()->XML_val('AfterClassLink', null, true);
$val .= '" ';

if ($scope->locally()->XML_val('RedirectionType', null, true)=='External') { 
$val .= ' data-icon="&#x23;" target="_blank"';


}
$val .= '>';

$val .= $scope->locally()->XML_val('Title', null, true);
$val .= '</a></h3>
					
					<p><strong>Date:</strong>
						';

$scope->locally()->obj('Dates', null, true); $scope->pushScope(); while (($key = $scope->next()) !== false) {
$val .= '
						';

$val .= $scope->locally()->obj('StartDateTime', null, true)->XML_val('Format', array('F j, Y'), true);
$val .= '<br />
						';


}; $scope->popScope(); 
$val .= '
						';

if ($scope->locally()->hasValue('Location', null, true)) { 
$val .= '
					<strong>Location:</strong> ';

$val .= $scope->locally()->XML_val('Location', null, true);
$val .= '</p>
					';


}
$val .= '
				<p>';

$val .= $scope->locally()->obj('Content', null, true)->XML_val('Summary', array('30'), true);
$val .= '
				</p>
				<span class="read-more"><a href="';

$val .= $scope->locally()->XML_val('AfterClassLink', null, true);
$val .= '" class="';

$val .= $scope->locally()->XML_val('ClassName', null, true);
$val .= '" data-icon="&#x23;" target="_blank" class="external"><span class="visuallyhidden">Information about ';

$val .= $scope->locally()->XML_val('Title', null, true);
$val .= '</span>';

if ($scope->locally()->hasValue('LinkShortName', null, true)) { 
$val .= $scope->locally()->XML_val('LinkShortName', null, true);

}else { 
$val .= 'More';


}
$val .= '</a></span>
				<div class="clearfix"></div>
				<a href="';

$val .= $scope->locally()->XML_val('AfterClassLink', null, true);
$val .= '" target="_blank"><span class="visuallyhidden">More information about ';

$val .= $scope->locally()->XML_val('Title', null, true);
$val .= '</span><div class="feature-content-overlay"></div></a>
			</div>
		</li>
		';


}; $scope->popScope(); 
$val .= '
	</ul>
	';


}else { 
$val .= '
	<h2>There aren\'t any events currently listed related to ';

$val .= $scope->locally()->XML_val('Title', null, true);
$val .= '.</h2>
	<p><a href="http://events.uiowa.edu/search/events/1?event_types%5B%5D=32570" target="_blank">See all Center For Student Involvement &amp; Leadership events &rarr;</a></p>
	';


}
$val .= '
</div>
</div>
<hr />
<div class="row month-content">

	';

if ($scope->locally()->hasValue('Content', null, true)) { 
$val .= '
		<div class="large-7 columns">
			';

$val .= $scope->locally()->XML_val('Content', null, true);
$val .= '
		</div>
	';


}
$val .= '

	<div class="';

if (!$scope->locally()->hasValue('Content', null, true)) { 
$val .= 'large-7 large-centered ';


}else { 
$val .= 'large-4 large-offset-1';


}
$val .= ' columns">
		<h2>Want to get involved?</h2>
		';

if ($scope->locally()->hasValue('EventTagTitle', null, true)) { 
$val .= '
		<p><a href="http://events.uiowa.edu/event/create" target="_blank">Submit your event to the UI Calendar</a> with the following general interest: "<strong>';

$val .= $scope->locally()->XML_val('EventTagTitle', null, true);
$val .= '</strong>" in order for it to be listed here!</p>
		<p><a href="http://events.uiowa.edu/event/create" target="_blank" class="btn">Submit an event</a></p>
		';


}
$val .= '
		<h3>Contact us</h3>
		<p><strong>Email: </strong><a href="mailto:getinvolved@uiowa.edu">getinvolved@uiowa.edu</a></p>
	</div>
</div>
';

$val .= SSViewer::execute_template('TopicsAndNews', $scope->getItem(), array(), $scope);

