<?php
$val .= '<div class="module">
    <div class="media">
    ';

if ($scope->locally()->hasValue('YouTubeEmbed', null, true)) { 
$val .= '
    	';

$val .= $scope->locally()->XML_val('YouTubeEmbed', null, true);
$val .= '
    ';


}else { 
$val .= '

    ';

if ($scope->locally()->hasValue('Image', null, true)) { 
$val .= '
        ';

if ($scope->locally()->hasValue('ExternalLink', null, true)) { 
$val .= '
          <a href="';

$val .= $scope->locally()->XML_val('ExternalLink', null, true);
$val .= '" target="_blank">
        ';


}else { 
$val .= '
            <a href="';

$val .= $scope->locally()->obj('AssociatedPage', null, true)->XML_val('Link', null, true);
$val .= '">
        ';


}
$val .= '
                <img src="data:image/png;base64," data-src="';

$val .= $scope->locally()->obj('Image', null, true)->obj('CroppedImage', array('359', '202'), true)->XML_val('URL', null, true);
$val .= '" alt="An image representing ';

$val .= $scope->locally()->XML_val('Title', null, true);
$val .= '">
            </a>
        ';


}
$val .= '
    ';


}
$val .= '
    </div>
    <div class="inner">
        <h3>
        ';

if ($scope->locally()->hasValue('ExternalLink', null, true)) { 
$val .= '
          <a href="';

$val .= $scope->locally()->XML_val('ExternalLink', null, true);
$val .= '" target="_blank">
        ';


}else { 
$val .= '
          <a href="';

$val .= $scope->locally()->obj('AssociatedPage', null, true)->XML_val('Link', null, true);
$val .= '">
        ';


}
$val .= '
        	';

$val .= $scope->locally()->XML_val('Title', null, true);
$val .= '</a>
      </h3>
        	';

$val .= $scope->locally()->XML_val('Content', null, true);
$val .= '
            ';

$val .= SSViewer::execute_template('HomePageFeatureFeedList', $scope->getItem(), array(), $scope);

$val .= '
    </div>
</div>
';

